...
export default class ProductShow extends Component {
  ...
  componentWillMount() {
    ...
    contentfulClient.getLocales().then(data => {
      ...
      const defaultLocale = data.items.filter(item => item.default === true)[0]
        .code;
      ...
      localeCodes.map(localeCode => {
        ...
        //     locale: 'en-US',
        //     "fields.slug": this.props.location.pathname.split("/")[1],
        //     include: 10
        //   })
        fetch(`https://9rukaawhx3.execute-api.us-east-1.amazonaws.com/default/contentfulexperiments?slug=twd-paradise-beach&locale=en-US&user=${window.username}`)
          .then(function(response) {
             return response.json();
           })
          .then(data => {
            console.log(data);
            productsByLocale['en-US'] = data.items[0];
  ...
  render() {
    ...
    document.title = this.state.productsByLocale[
      this.state.currentLocale
    ].fields.title;

    let sections = this.state.productsByLocale[
      this.state.currentLocale
